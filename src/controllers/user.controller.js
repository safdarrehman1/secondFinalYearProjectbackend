...
const getUserByIdPublic = catchAsync(async (req, res) => {
  ...
  const virtualFollowers = user.virtualFollowers || 0;
  ...
  // Override only followers with hardcoded stats for test accounts; totalLikes always from live aggregate so profile matches user-space works
  if (user && user.testAccount) {
    const userIdStr = user._id.toString();
    if (hardcodedStats[userIdStr] && hardcodedStats[userIdStr].followers != null) {
      result.followers = hardcodedStats[userIdStr].followers;
    }
  }
