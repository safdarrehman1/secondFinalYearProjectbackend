...
const { musicService, userSpaceService, userStatsService } = require("../services");
...
const getAllCreationsFollowing = catchAsync(async (req, res) => {
  let userId = null;
  if (req.user && req.user.id) {
    userId = req.user.id;
  }
  const creations = await musicService.getAllCreationsFollowing(userId);
  if (!creations) {
    throw new ApiError(httpStatus.NOT_FOUND, "Creations not found");
  }
  res.send(creations);
});
  ...
  const usersWithSpace = (
    await Promise.all(
      users.map(async (user) => {
        ...
        // Same totalLikes as profile/user-space (all works + gig favorites)
        const totalLikes = await userStatsService.calculateTotalLikes(user.id);
