...
const createExtraPaymentPaypalOrder = async (req, res) => {
  try {
    ...
    // Calculate fees for extra payment
    const { calculateBuyerPayment } = require("../utils/feeCalculator");
    const buyerPayment = calculateBuyerPayment(request.amount);
    ...
    console.error("Create Extra Payment PayPal Order Error:", error);
    res
      .status(error.statusCode || httpStatus.BAD_REQUEST)
      .send({ message: error.message });
  }
...
const requestExtraPayment = async (req, res) => {
  try {
    const { orderId } = req.params;
    const { amount, message } = req.body;
    const userId = req.user._id;

    const updatedOrder = await orderService.requestExtraPayment(
      orderId,
      amount,
      message,
      userId,
...
const decideExtraPayment = async (req, res) => {
  try {
    const { orderId, requestId } = req.params;
    ...
    if (status !== "declined") {
      throw new ApiError(
        httpStatus.BAD_REQUEST,
        "Only 'declined' status is allowed via this endpoint",
      );
    }

    const updatedOrder = await orderService.decideExtraPayment(
      orderId,
      requestId,
      status,
      userId,
...
const payExtraPayment = async (req, res) => {
  try {
    const { orderId, requestId } = req.params;
    const { paypalOrderId } = req.body; // Represents the successful PayPal capture ID or Order ID
    const userId = req.user._id;

    const updatedOrder = await orderService.payExtraPayment(
      orderId,
      requestId,
      paypalOrderId,
      userId,
  ...
  try {
    ...
    try {
      await notificationService.createNotification(
        ...
        order.recruiterId,
        userId,
        `Delivery submitted for order: ${order.title}`,
        { orderId: order._id },
      );
    } catch (error) {
...
const adminAcceptCancellation = async (req, res) => {
  try {
    ...
    if (!adminReviewCancellation) {
      // Check for pending support request if no admin_review cancellation exists
      if (order.requestSupport && order.requestSupport.status === "pending") {
        // Handle support request acceptance (Cancel Order)
        order.requestSupport.status = "resolved";
        order.requestSupport.resolution =
          "Order Cancelled by Admin decision on support request";
        order.requestSupport.resolvedAt = new Date();
        order.requestSupport.resolvedBy = adminId;

        // Continue to cancellation logic below...
      } else {
        return res.status(httpStatus.BAD_REQUEST).send({
          success: false,
          message:
            "No cancellation request under admin review or pending support request found for this order",
        });
      }
    } else {
      // Standard cancellation acceptance
    ...
    // Update cancellation status to accepted if it fits standard flow (already done above for cleaner logic flow but keeping consistent)
    // We handled adminReviewCancellation updates above to distinguish from support request flow

    // Add activity log with context
    const isSupportRequest = !adminReviewCancellation;
    const activityAction = isSupportRequest
      ? "support_resolved_cancellation_admin"
      : "cancellation_accepted_by_admin";
    const cancellationIdMeta = adminReviewCancellation
      ? adminReviewCancellation._id
      : null;
    ...
    order.activities.push({
      ...
      note:
        adminReason ||
        (isSupportRequest
          ? "Admin cancelled order to resolve support request"
          : "Admin approved cancellation request"),
      fromStatus: order.status,
      toStatus: "cancel",
      meta: {
        refundProcessed: hasPayment,
        cancellationId: cancellationIdMeta,
      },
    });
    ...
    // Also resolve any pending support request if this was a normal cancellation decision
    if (
      adminReviewCancellation &&
      order.requestSupport &&
      order.requestSupport.status === "pending"
    ) {
      order.requestSupport.status = "resolved";
      order.requestSupport.resolution = "Resolved via Cancellation Acceptance";
      order.requestSupport.resolvedAt = new Date();
      order.requestSupport.resolvedBy = adminId;
    }
    ...
    try {
      ...
      if (order.createdBy) {
        await notificationService.createNotification(
          ...
          { orderId: order._id, reason: adminReason || "Admin decision" },
        );
      }
      if (order.recruiterId) {
        await notificationService.createNotification(
      ...
      console.error(
        "Failed to send notification for admin cancellation:",
        error,
      );
    }
...
const adminRejectCancellation = async (req, res) => {
  try {
    ...
    if (!adminReviewCancellation) {
      // Check for pending support request
      if (order.requestSupport && order.requestSupport.status === "pending") {
        // Handle support request rejection (Keep Order Active)
        order.requestSupport.status = "resolved";
        order.requestSupport.resolution =
          "Cancellation/Support Rejected by Admin";
        order.requestSupport.resolvedAt = new Date();
        order.requestSupport.resolvedBy = adminId;
        ...
        order.activities = order.activities || [];
        order.activities.push({
          action: "support_resolved_kept_active_admin",
          by: adminId,
          note: `Admin action: support request rejected by Admin. Admin message: ${
            adminReason ||
            "Admin resolved support request and kept order active"
          }`,
          meta: {
            originalReason: order.requestSupport.reason,
          },
        });
        ...
        return res.status(httpStatus.OK).send({
          success: true,
          message: "Support request resolved (Order kept active)",
          order: {
            id: order._id,
            status: order.status,
          },
    ...
    // Also resolve any pending support request
    if (order.requestSupport && order.requestSupport.status === "pending") {
      order.requestSupport.status = "resolved";
      order.requestSupport.resolution = "Resolved via Cancellation Rejection";
      order.requestSupport.resolvedAt = new Date();
      order.requestSupport.resolvedBy = adminId;
    }
    ...
    // Notification: Admin Rejected Cancellation
    try {
      // We need to fetch requestedBy if it's just an ID
      const requesterId = adminReviewCancellation.requestedBy;
      if (requesterId) {
        await notificationService.createNotification(
          "cancellation_declined",
          requesterId,
          adminId,
          `Cancellation request declined by admin: ${order.title}`,
          { orderId: order._id, reason: adminReason || "Admin decision" },
        );
      }
    } catch (error) {
      console.error("Failed to send notification for admin rejection:", error);
    }
...
const requestDeliveryRevision = async (req, res) => {
  try {
    ...
    // Send notification
    try {
      // Determine recipient - opposite of who requested revision
      const recipientId =
        uid === createdByStr ? order.recruiterId : order.createdBy;

      await notificationService.createNotification(
        "delivery_revision",
        recipientId,
        userId,
        `Revision requested for order: ${order.title}`,
        { orderId: order._id },
      );
    } catch (error) {
...
// Request Support on Order (Arbitration)
  ...
  try {
    ...
    if (userIdStr !== createdByStr && userIdStr !== recruiterStr) {
      return res.status(httpStatus.FORBIDDEN).send({
        success: false,
        message: "Not authorized to request support for this order",
      });
    }

    // Update Request Support
    order.requestSupport = {
      initiatedBy: userId,
    ...
    order.activities.push({
      ...
      by: userId,
      note: `Support requested: ${reason}`,
      // note: `Support requested: ${reason} by ${requesterName}`,
      meta: { description, reason },
    });

    await order.save();
    return res.status(httpStatus.OK).send({ success: true, order });
  } catch (error) {
...
const adminResolveSupport = async (req, res) => {
  try {
    const { orderId } = req.params;
    ...
    if (!order.requestSupport || order.requestSupport.status !== "pending") {
      return res.status(httpStatus.BAD_REQUEST).send({
        success: false,
        message: "No pending support request found",
      });
    }

    order.requestSupport.status = "resolved";
    order.requestSupport.resolution = note;
    order.requestSupport.resolvedBy = adminId;
    order.requestSupport.resolvedAt = new Date();
    ...
    order.activities.push({
      ...
      by: adminId,
      note: `Admin action: support request resolved by Admin. Admin message: ${note}`,
    ...
    console.error("Admin resolve support error:", error);
    return res
      .status(httpStatus.BAD_REQUEST)
      .send({ success: false, message: error.message });
  }
