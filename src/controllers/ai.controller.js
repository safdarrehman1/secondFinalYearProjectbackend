...
const extractResumeText = async (file) => {
  ...
  throw new Error(
    "Unsupported resume format. Upload a PDF, DOC, DOCX, or TXT file.",
  );
};
...
const analyzeResumeLocally = (job, resumeText) => {
  ...
  (job.designSubcategory || []).forEach((subcategory) =>
    addTerms(subcategory, 5),
  );
  ...
  const rankedTerms = [...weightedTerms.entries()].sort(
...
Return ONLY valid JSON in this exact format:
{
  ...
  try {
    ...
    if (!result.description || result.description.length < 100) {
      throw new Error(
        "Invalid AI response: description too short (minimum 100 characters)",
      );
    }
    ...
    // Log success
    console.log(
      `AI Autofill Success - User: ${req.user.id}, Response Time: ${responseTime}ms`,
    );

    console.log(
      `AI API - Tokens: ${
        completion.usage?.totalTokenCount || "unknown"
      }, Model: ${
        config.aiProvider === "groq" ? config.groq.model : config.gemini.model
      }`,
    );
  ...
  try {
    ...
    if (resumeText.length < 80) {
      return res.status(httpStatus.BAD_REQUEST).json({
        success: false,
        message:
          "The uploaded resume does not contain enough readable text. Please upload a text-based resume.",
      });
    }

    const applicantAccount = await User.findById(req.user.id)
      .select("name")
      .lean();
    try {
      await applicationService.validateResumeAuthenticity(
        resumeText,
    ...
    if (typeof previousAnalysis === "string" && previousAnalysis.trim()) {
      try {
        ...
        return res
          .status(httpStatus.BAD_REQUEST)
          .json({ message: "previousAnalysis must be valid JSON" });
      }
    }
...
${
  hasPreviousSubmission
    ? previousResume.slice(0, MAX_RESUME_CHARACTERS)
    : "Not provided"
}
...
Return ONLY valid JSON in this exact shape, no preamble:
{
  ...
  "cooldown_required_days": null
    ...
    let result;
    if (
      config.aiProvider === "groq" ? config.groq.apiKey : config.gemini.apiKey
    ) {
      try {
        const completion = await generateGeminiContent(
        ...
        if (
          !Number.isFinite(Number(aiResult.match_score)) ||
          !Array.isArray(aiResult.matched_skills)
        ) {
        ...
        console.warn(
          `Resume Match AI unavailable, using local analysis: ${error.message}`,
        );
      }
    }
    ...
    const score = Math.max(
      0,
      Math.min(100, Math.round(Number(result.match_score) || 0)),
    );
    ...
    const gamingFlag = flags.includes(result.gaming_flag)
      ? result.gaming_flag
      : "not_applicable";
    const eligibleToApply =
      score > threshold &&
      unverifiedClaims.length === 0 &&
      gamingFlag !== "keyword_stuffing";
    let suggestions = Array.isArray(result.improvement_suggestions)
      ? result.improvement_suggestions
      : [];
    let suggestion =
      suggestions
        .map((item) => item.how_to_gain_it)
        .filter(Boolean)
        .join(" ") || buildLocalSuggestion(score, getOutcome(score), []);
    let screening = null;
    try {
      screening = await screeningService.parseResume(
        resumeText,
        job.description,
        req.user.id,
      );
      ...
      console.warn(
        `Applicant screening metadata unavailable; continuing legacy resume flow: ${error.message}`,
      );
    }
    ...
    if (screening?.suggestedDescription)
      suggestion = screening.suggestedDescription;
    let resumeFile = null;
    try {
      ...
      console.warn(
        `Resume persistence unavailable; continuing without attachment: ${error.message}`,
      );
    }
    ...
    return res.status(httpStatus.OK).json({
      ...
      suggestion:
        suggestion ||
        buildLocalSuggestion(score, getOutcome(score), missingSkills),
      match_score: score,
    ...
    return res.status(httpStatus.BAD_REQUEST).json({
      ...
      message:
        error.message ||
        "Unable to analyze the resume. Please try again with a readable resume file.",
    });
  }
});
...
const generateApplicationMessage = catchAsync(async (req, res) => {
  ...
  try {
    const completion = await generateGeminiContent(
      ...
      `Write a unique, personalized application message from an applicant to ${
        recipientName || "the recruiter"
      } for this job.
Job title: ${job.projectTitle}
...
Skills the applicant should improve: ${
...
Use the role details to make the message specific. Do not invent employers, years of experience, qualifications, or projects. Mention relevant fit confidently but honestly. Vary the opening and sentence structure naturally. Write 80 to 140 words. Return only JSON: {"message":"..."}`,
    ...
    const generated = parsed.message || parsed.applicationMessage || parsed.text || completion.text;
    ...
    return res.status(503).json({
      success: false,
      message:
        "AI could not generate the application message right now. Please try again.",
...
Use the supplied details to create natural, specific prose with varied sentence structure. Use only the supplied facts; do not invent employers, qualifications, awards, clients, or years of experience. Keep it warm and professional in 90 to 150 words. Return only JSON: {"aboutMe":"..."}`,
    ...
    const parsed = extractJson(completion.text);
    const generated = parsed.aboutMe || parsed.about || parsed.text || completion.text;
    if (typeof generated !== "string" || generated.trim().length < 30) {
      throw new Error("Invalid AI response: profile biography is too short");
    }
    return res
      .status(httpStatus.OK)
      .json({ success: true, aboutMe: generated.trim().slice(0, 3000) });
  } catch (error) {
    console.error(`Profile About AI generation failed: ${error.message}`);
    return res.status(503).json({
      success: false,
      message:
        "AI could not generate the profile description right now. Please try again.",
      error: "AI_GENERATION_FAILED",
    });
  }
});
...
Adapt the structure and wording to this specific role. Include a concise overview, concrete responsibilities, required skills and tools, preferred experience, and expected outcomes only when supported by the prompt. Write 250 to 450 words. Return only JSON: {"description":"..."}`,
      { temperature: 0.7, json: true },
      req.user?.id,
      req.originalUrl || "/api/job-description",
    );
    const parsed = extractJson(completion.text);
    const generated = parsed.description || parsed.jobDescription || parsed.text || completion.text;
    if (typeof generated !== "string" || generated.trim().length < 30) {
      throw new Error("Invalid AI response: job description is too short");
    }
    return res
      .status(httpStatus.OK)
      .json({ success: true, description: generated.trim().slice(0, 5000) });
  } catch (error) {
    console.error(`Job description AI generation failed: ${error.message}`);
    return res.status(503).json({
      success: false,
      message:
        "AI could not generate the job description right now. Please try again.",
      error: "AI_GENERATION_FAILED",
    });
  }
});
