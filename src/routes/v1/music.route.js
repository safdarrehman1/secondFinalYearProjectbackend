...
router.route("/upload").post(auth(), musicController.uploadMusic);
...
router
  ...
  .get(
    auth(),
    validate(musicValidation.getMusicBox),
    musicController.getMusicBox,
  );
...
router
  .route("/get-all-music-following")
  .get(auth(), musicController.getAllMusicFollowing);
router
  .route("/get-all-creations-following")
  .get(auth(), musicController.getAllCreationsFollowing);
router
  .route("/get-music/:id")
  .get(
    optionalAuth(),
...
router
  .route("/delete-music/:musicId")
  .delete(auth(), musicController.deleteMusic);
...
router
  .route("/view/:musicId")
  .post(optionalAuth(), musicController.incrementView);
router
  .route("/comment/:musicId")
  .post(
    auth(),
    validate(musicValidation.commentMusic),
    musicController.commentOnMusic,
  );
router
  .route("/foryou")
  .get(auth(), recommendationController.getRecommendations);
...
router
  .route("/search/all")
  .get(optionalAuth(), musicController.searchAllAssets);

// Get all contributions for a specific song
router
  .route("/contributions/:songName")
  .get(optionalAuth(), musicController.getContributionsForSong);

// Add contributor to existing music
router
  .route("/contribute/:musicId")
  .post(auth(), musicController.addContributor);
