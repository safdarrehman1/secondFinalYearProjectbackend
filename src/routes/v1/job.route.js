...
router.route("/upload-images").post(
  ...
  catchAsync(async (req, res) => {
    if (
      !req.files ||
      (!req.files["applicantAvatar"] && !req.files["applicantBackgroundImage"])
    ) {
    ...
    if (req.files["applicantAvatar"]) {
      const avatarResult = await uploadFileToS3(
        req.files["applicantAvatar"][0],
        req.user.id,
      );
    ...
    if (req.files["applicantBackgroundImage"]) {
      const backgroundResult = await uploadFileToS3(
        req.files["applicantBackgroundImage"][0],
        req.user.id,
      );
...
router.get(
  ...
  auth(),
  jobController.getJobWithApplicants,
);
...
// Extend job route
router.post("/:jobId/extend", auth(), jobController.extendJob);

// New DELETE route to delete a job
router
  .route("/:jobId")
  .delete(validate(auth(), jobValidation.getJobById), jobController.deleteJob);

// New PUT route to update a jobrecruiters
router
  .route("/:jobId")
  .put(auth(), validate(jobValidation.getJobById), jobController.updateJob);
...
router.get(
  "/:jobId",
  auth(),
  validate(jobValidation.getJobById),
