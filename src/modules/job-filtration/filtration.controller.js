...
const FiltrationJob = require("./filtration-job.model");
...
const ownJob = async (id, userId) => {
  ...
  if (!job) job = await FiltrationJob.findById(id);
  if (!job) throw new ApiError(httpStatus.NOT_FOUND, "Job not found");
  if (String(job.poster || job.createdBy) !== String(userId)) throw new ApiError(httpStatus.FORBIDDEN, "Only the poster can manage this job");
  return job;
};

exports.createJob = async (req, res) => successResponse(res, await FiltrationJob.create({ ...req.body, poster: req.user.id }), "Job draft created", 201);
exports.uploadResume = async (req, res) => {
  ...
  const applicantAccount = await User.findById(req.user.id).select("name").lean();
  try {
    await applicationService.validateResumeAuthenticity(extractedText, applicantAccount?.name, req.user.id);
  } catch (error) {
...
exports.listJobs = async (req, res) => {
  ...
  if (req.query.type === "gig") {
    typeCondition = {
      $or: [
        ...
        { jobType: { $in: ["Freelance", "Gig", "Contract"] } },
      ],
    };
  } else if (req.query.type === "full_time") {
    typeCondition = {
      $or: [
        { type: "full_time" },
        { employmentType: { $in: ["full-time", "part-time", "permanent"] } },
  ...
  const [mainItems, mainTotal, filtItems, filtTotal] = await Promise.all([
    Job.find(query).sort("-createdAt").lean(),
    Job.countDocuments(query),
    FiltrationJob.find(query).sort("-createdAt").lean(),
    FiltrationJob.countDocuments(query),
  ]);

  const allRaw = [...mainItems, ...filtItems].sort((a, b) => new Date(b.createdAt || b.createdOn || 0).getTime() - new Date(a.createdAt || a.createdOn || 0).getTime());
  const total = mainTotal + filtTotal;
  const paginated = allRaw.slice((page - 1) * limit, page * limit);

  const normalizedItems = paginated.map((jobObj) => {
    const isGig =
      jobObj.type === "gig" ||
      jobObj.employmentType === "freelance-project" ||
      (Array.isArray(jobObj.jobType) && jobObj.jobType.some((t) => /freelance|gig|contract/i.test(t)));

    let formattedLocation = jobObj.preferredLocation || jobObj.fullTimeDetails?.location || "Remote";
    if (Array.isArray(jobObj.jobType) && jobObj.jobType.includes("Remote")) {
      formattedLocation = "Remote";
    }

    let budgetOrSalary = "";
    if (isGig) {
      budgetOrSalary = jobObj.gigDetails?.budget
        ? `$${jobObj.gigDetails.budget}`
        : jobObj.budget
        ? (String(jobObj.budget).startsWith("$") ? String(jobObj.budget) : `$${jobObj.budget}`)
        : "Budget TBD";
    } else {
      if (jobObj.fullTimeDetails?.salaryMin && jobObj.fullTimeDetails?.salaryMax) {
        budgetOrSalary = `$${jobObj.fullTimeDetails.salaryMin}–$${jobObj.fullTimeDetails.salaryMax}`;
      } else if (jobObj.salaryRange?.min && jobObj.salaryRange?.max) {
        budgetOrSalary = `${jobObj.salaryRange.currency || "$"}${jobObj.salaryRange.min}–${jobObj.salaryRange.max}`;
      } else if (jobObj.budget) {
        budgetOrSalary = String(jobObj.budget).startsWith("$") ? String(jobObj.budget) : `$${jobObj.budget}`;
      } else {
        budgetOrSalary = "Salary TBD";
      }
    }

    return {
      ...jobObj,
      _id: jobObj._id.toString(),
      title: jobObj.projectTitle || jobObj.title || jobObj.position || "Untitled Position",
      description: jobObj.description || "No description provided.",
      type: isGig ? "gig" : "full_time",
      employmentTypeLabel: isGig ? "Freelance Project" : "Full-Time Job",
      formattedLocation,
      budgetOrSalary,
    };
  });

  return successResponse(res, { items: normalizedItems, page, limit, total }, "Published jobs");
};

exports.getJob = async (req, res) => {
  let job = await Job.findById(req.params.id).lean();
  if (!job) {
    job = await FiltrationJob.findById(req.params.id).lean();
  }
  if (!job || ["draft", "scheduled", "archived", "inactive"].includes(job.status)) {
    throw new ApiError(404, "Job not found");
  }

  const isGig =
    job.type === "gig" ||
    job.employmentType === "freelance-project" ||
    (Array.isArray(job.jobType) && job.jobType.some((t) => /freelance|gig|contract/i.test(t)));

  let formattedLocation = job.preferredLocation || job.fullTimeDetails?.location || "Remote";
  if (Array.isArray(job.jobType) && job.jobType.includes("Remote")) {
    formattedLocation = "Remote";
  }

  let budgetOrSalary = "";
  if (isGig) {
    budgetOrSalary = job.gigDetails?.budget
      ? `$${job.gigDetails.budget}`
      : job.budget
      ? (String(job.budget).startsWith("$") ? String(job.budget) : `$${job.budget}`)
      : "Budget TBD";
  } else {
    if (job.fullTimeDetails?.salaryMin && job.fullTimeDetails?.salaryMax) {
      budgetOrSalary = `$${job.fullTimeDetails.salaryMin}–$${job.fullTimeDetails.salaryMax}`;
    } else if (job.salaryRange?.min && job.salaryRange?.max) {
      budgetOrSalary = `${job.salaryRange.currency || "$"}${job.salaryRange.min}–${job.salaryRange.max}`;
    } else if (job.budget) {
      budgetOrSalary = String(job.budget).startsWith("$") ? String(job.budget) : `$${job.budget}`;
    } else {
      budgetOrSalary = "Salary TBD";
    }
  }
  ...
  const normalized = {
    ...
    title: job.projectTitle || job.title || job.position || "Untitled Position",
...
exports.apply = async (req, res) => {
  ...
  let isFiltration = false;
  if (!job) {
    job = await FiltrationJob.findById(req.params.id);
    isFiltration = true;
  }
  ...
  if (!["active", "published"].includes(job.status)) {
    throw new ApiError(409, `This job is not accepting applications (${job.status})`);
  }
  ...
  application.resumeStatus = result.requiresManualReview ? "pending" : (result.weighted >= (job.minResumePct || 60) ? "passed" : "failed");
  ...
  else if (application.resumeStatus === "passed") applicationStatus.changeStatus(application, (job.type === "gig" || job.employmentType === "freelance-project") ? "shortlisted" : "resume_screened", req.user.id, "Resume gate passed");
  else {
    applicationStatus.changeStatus(application, "rejected", req.user.id, "Resume score did not meet the required threshold");
    application.reapplyAfter = new Date(Date.now() + (job.cooldownDays || 7) * 86400000);
...
exports.submitTest = async (req, res) => {
  ...
  applicationStatus.changeStatus(application, result.requiresManualReview ? "under_review" : (result.score >= (application.job.minTestPct || 60) ? (application.job.type === "gig" || application.job.employmentType === "freelance-project" ? "shortlisted" : "test_completed") : "rejected"), req.user.id, result.requiresManualReview ? "Task assessment queued for manual scoring" : (result.score >= (application.job.minTestPct || 60) ? "Test gate passed" : "Test score did not meet the required threshold"));
  if (application.finalStatus === "rejected") application.reapplyAfter = new Date(Date.now() + (application.job.cooldownDays || 7) * 86400000);
