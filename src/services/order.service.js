...
const createOrder = async (orderData) => {
  ...
  try {
    if (order.createdBy && order.recruiterId) {
      // Assuming buyer (recruiter) creates order for seller (createdBy)
      await notificationService.createNotification(
        "order_created",
        order.createdBy, // Receiver: Seller
        order.recruiterId, // Sender: Buyer
        `New order received: ${order.title}`,
        { orderId: order._id },
      );
    }
  } catch (error) {
    console.error("Failed to send notification for order creation:", error);
  }
...
const getOrderById = async (orderId, currentUser) => {
  ...
  orderData.activities = await Promise.all(
    (order.activities || []).map(async (activity) => {
      if (activity.by) {
        const byIdStr = activity.by.toString();

        // Try to get info from UserSpace first
        ...
        let lastName =
          userSpace && userSpace.lastName ? userSpace.lastName : "";

        // Fallback to User model if name is still missing
        if (!firstName) {
          const user = await User.findById(byIdStr).select("name").lean();
          if (user && user.name) {
            firstName = user.name;
          }
        }
...
const updateOrderStatus = async (
  ...
  try {
    const recipient =
      actorId && actorId.toString() === order.createdBy.toString()
        ? order.recruiterId // If actor is seller, notify buyer
        : order.createdBy; // If actor is buyer, notify seller

    const sender =
      actorId ||
      (recipient.toString() === order.createdBy.toString()
        ? order.recruiterId
        : order.createdBy);

    let notifType = "order_status_update";
    let notifMsg = `Order status updated to ${status}`;

    if (status === "accepted") {
      notifType = "order_accepted";
      notifMsg = `Order accepted: ${order.title}`;
    } else if (status === "delivered") {
      notifType = "order_delivered";
      notifMsg = `Order delivered: ${order.title}`;
    } else if (status === "revision") {
      notifType = "delivery_revision";
      notifMsg = `Revision requested: ${order.title}`;
    } else if (status === "complete") {
      notifType = "order_completed";
      notifMsg = `Order completed: ${order.title}`;
    } else if (status === "cancel") {
      notifType = "order_cancelled";
      notifMsg = `Order cancelled: ${order.title}`;
    }
    ...
    console.error("Failed to send notification for order update:", error);
  }
  ...
  try {
    if (order.createdBy) {
      await notificationService.createNotification(
        "order_review",
        order.createdBy, // Receiver: Seller
        order.recruiterId || actorId, // Sender: Buyer
        `New review received for order: ${order.title}`,
        { orderId: order._id, rating: rating || buyerRating },
      );
    }
  } catch (error) {
    console.error("Failed to send notification for review:", error);
  }
...
const submitReviewReply = async (orderId, reply, actorId) => {
  ...
  // Notification: Review Reply
  try {
    if (order.recruiterId) {
      await notificationService.createNotification(
        "review_reply",
        order.recruiterId, // Receiver: Buyer
        order.createdBy, // Sender: Seller
        `Seller replied to your review on: ${order.title}`,
        { orderId: order._id },
      );
    }
  } catch (error) {
...
module.exports = {
  ...
  async extendDelivery(orderId, extraDays, actorId) {
    ...
    // Notification: Delivery Extended (Direct)
    try {
      const recipient = order.recruiterId; // Notify buyer
      if (recipient) {
        await notificationService.createNotification(
          "extension_accepted",
          recipient,
          actorId || order.createdBy,
          `Delivery time extended by ${extraDays} days for: ${order.title}`,
          { orderId: order._id },
        );
      }
    } catch (error) {
      console.error(
        "Failed to send notification for delivery extension:",
        error,
      );
    }
  ...
  async requestExtension(orderId, days, reason, actorId) {
    ...
    // Notification: Extension Requested
    try {
      const recipient =
        actorId.toString() === order.createdBy.toString()
          ? order.recruiterId
          : order.createdBy;
      ...
      if (recipient) {
        await notificationService.createNotification(
          ...
          recipient,
          actorId,
          `Extension requested for: ${order.title}`,
          { orderId: order._id, days, reason },
        );
      }
    } catch (error) {
      console.error(
        "Failed to send notification for extension request:",
        error,
      );
    }
  ...
  async decideExtension(orderId, extensionIndex, decision, deciderId) {
    ...
    // Notification: Extension Decision
    try {
      const recipient =
        deciderId.toString() === order.createdBy.toString()
          ? order.recruiterId
          : order.createdBy;

      const notifType =
        decision === "accepted" ? "extension_accepted" : "extension_declined";
      const notifMsg =
        decision === "accepted"
          ? `Extension request accepted for: ${order.title}`
          : `Extension request declined for: ${order.title}`;
      ...
      console.error(
        "Failed to send notification for extension decision:",
        error,
      );
    }
  ...
  async requestCancellation(orderId, reason, actorId, attachments = []) {
    ...
    // Notification: Cancellation Requested
    try {
      const recipient =
        actorId.toString() === order.createdBy.toString()
          ? order.recruiterId
      ...
      if (recipient) {
        await notificationService.createNotification(
          ...
          recipient,
          actorId,
          `Cancellation requested for: ${order.title}`,
          { orderId: order._id, reason },
        );
      }
    } catch (error) {
      console.error(
        "Failed to send notification for cancellation request:",
        error,
      );
    }
  ...
  async decideLatestCancel(orderId, decision, deciderId) {
    ...
    if (decision === "accepted") {
      ...
      order.activities.push({
        ...
        by: deciderId,
        note: `Cancellation declined by ${deciderName}.`,
        meta: {
    ...
    try {
      // The recipient should be the person who REQUESTED the cancellation
      // The decider is the person acting now (accepting/declining)
      const recipient = req.requestedBy;
      ...
      const notifMsg =
        decision === "accepted"
          ? `Cancellation request accepted for: ${order.title}`
          : `Cancellation request declined for: ${order.title}`;
      ...
      console.error(
        "Failed to send notification for cancellation decision:",
        error,
      );
    }
  ...
  async directCancelOrder(orderId, reason, actorId) {
    ...
    try {
      ...
      // Also notify the other party if admin did it
      const isParticipant =
        actorId.toString() === order.createdBy.toString() ||
        actorId.toString() === order.recruiterId.toString();
      ...
      if (isParticipant && recipient) {
        await notificationService.createNotification(
          ...
          recipient,
          actorId,
          `Order cancelled directly: ${order.title}`,
          { orderId: order._id, reason: reason || "Direct cancellation" },
        );
        ...
        if (order.createdBy) {
          await notificationService.createNotification(
            ...
            order.createdBy,
            actorId,
            `Order cancelled by admin: ${order.title}`,
            { orderId: order._id, reason: reason || "Admin cancellation" },
          );
        }
        if (order.recruiterId) {
          await notificationService.createNotification(
      ...
      console.error(
        "Failed to send notification for direct cancellation:",
        error,
      );
    }
  ...
  async requestExtraPayment(orderId, amount, message, actorId) {
    ...
    // Only seller (createdBy) can request extra payment
    if (actorId.toString() !== order.createdBy.toString()) {
      throw new ApiError(
        httpStatus.FORBIDDEN,
        "Only the seller can request extra payment",
      );
    }

    order.extraPaymentRequests = order.extraPaymentRequests || [];
    order.extraPaymentRequests.push({
    ...
    // Log activity
    order.activities.push({
      action: "extra_payment_requested",
      by: actorId,
      note: `Requested extra payment of $${amount}`,
      meta: { amount, message, requestId: order.extraPaymentRequests[order.extraPaymentRequests.length - 1]._id },
    });
    ...
    try {
      if (order.recruiterId) {
        await notificationService.createNotification(
          ...
          order.recruiterId,
          actorId,
          `Seller requested extra payment of $${amount} for: ${order.title}`,
          { orderId: order._id },
        );
      }
    } catch (error) {
      console.error("Failed notification for extra payment request", error);
    }

    return order;
  },
  async decideExtraPayment(orderId, requestId, status, actorId) {
    const order = await Order.findById(orderId);
    if (!order) throw new ApiError(httpStatus.NOT_FOUND, "Order not found");

    const request = order.extraPaymentRequests.id(requestId);
    ...
    if (
      ...
      throw new ApiError(
        httpStatus.FORBIDDEN,
        "Only the buyer can decline this request",
      );
    }
    ...
    order.activities.push({
      action: "extra_payment_decided",
      by: actorId,
      note: `Extra payment request ${status}`,
      meta: { requestId: requestId.toString(), status, amount: request.amount },
    });
    ...
    try {
      if (order.createdBy) {
        await notificationService.createNotification(
          ...
          order.createdBy,
          actorId,
          `Extra payment request was ${status} for: ${order.title}`,
          { orderId: order._id, status },
        );
      }
    } catch (error) {
      console.error("Failed notification for extra payment decide", error);
    }

    return order;
  },
  async payExtraPayment(orderId, requestId, paypalOrderId, actorId) {
    const order = await Order.findById(orderId);
    if (!order) throw new ApiError(httpStatus.NOT_FOUND, "Order not found");
    ...
    request.status = "paid";
    request.paymentId = captureData.id; // Capture ID
    request.decidedBy = actorId; // Buyer paid it
    request.decidedAt = new Date();

    // Update Order Total Amount
    order.totalAmount = (order.totalAmount || 0) + request.amount;

    // Log Activity
    order.activities.push({
      action: "extra_payment_paid",
      by: actorId,
      note: `Extra payment of $${request.amount} paid`,
      meta: {
    ...
    try {
      if (order.createdBy) {
        await notificationService.createNotification(
          ...
          order.createdBy,
          actorId,
          `Extra payment of $${request.amount} received for: ${order.title}`,
          { orderId: order._id, amount: request.amount },
        );
      }
    } catch (error) {
      console.error("Failed notification for extra payment paid", error);
    }
