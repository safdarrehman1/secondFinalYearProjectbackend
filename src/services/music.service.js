...
const getMusicById = async (id, userId = null) => {
  ...
  if (music) {
    ...
    if (user && Array.isArray(user.following)) {
      isFollowing = user.following.some(
        (followingId) => followingId.toString() === obj.createdBy.toString(),
      );
    }
  ...
  if (lyric) {
    ...
    if (user && Array.isArray(user.following)) {
      isFollowing = user.following.some(
        (followingId) => followingId.toString() === lyric.createdBy.toString(),
      );
    }
  ...
  if (creation) {
    ...
    return {
      ...
      musicImage:
        creation.musicImage ||
        (Array.isArray(creation.workImages) && creation.workImages.length > 0
          ? creation.workImages[0]
          : ""),
...
const getMyMusic = async (userId) => {
  ...
  const music = await Music.find({ createdBy: userId }).populate(
    "contributors.userId",
  ...
  const normalizedMusic = music.map((item) => {
    ...
    if (
      obj.contributors &&
      Array.isArray(obj.contributors) &&
      ...
      const otherContributors = obj.contributors.map((contributor) => ({
        ...
        userName:
          contributor.userName || contributor.userId?.name || "Unknown User",
        profilePicture:
          contributor.profilePicture ||
          contributor.userId?.profilePicture ||
          "",
...
const getAllMusicFollowing = async (userId) => {
  ...
  }).populate("contributors.userId", "name email profilePicture");
  const lyricsMusic = await LyricsMusic.find({
    createdBy: { $in: user.following },
  });
...
/**
 * Get all music creations from users that the current user is following
 * @param {string} userId
 * @returns {Promise<Array>}
 */
const getAllCreationsFollowing = async (userId) => {
  if (!userId) return [];
  const user = await User.findById(userId).select("following").lean();
  if (!user || !user.following || user.following.length === 0) return [];
  ...
  // Collect all unique createdBy user IDs
  const allUserIds = [...new Set(creations.map((item) => item.createdBy))];
  const userSpaces = await UserSpace.find({
    createdBy: { $in: allUserIds },
  }).lean();
  ...
  const normalizedCreations = creations.map((item) => {
    const userInfo = userMap[item.createdBy] || {};
    let isLiked = false;
    ...
    return {
      ...
      musicImage:
        item.musicImage ||
        (Array.isArray(item.workImages) && item.workImages.length > 0
          ? item.workImages[0]
          : "/image/default-picture.jpg"),
      // Ensure compatibility with HomeMusicianBox
      songName: item.title,
      composerName: userInfo.userName || "",
    };
  });

  return normalizedCreations.sort(
