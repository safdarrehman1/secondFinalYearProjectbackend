 ...
 */
const validateGigAgainstProfile = (
  category,
  subcategory,
  creationOccupation = [],
  businessOccupation = "",
  ...
  const normalizedExpectedOccupations = expectedOccupations.map((occ) =>
    occ.toLowerCase().trim(),
  );

  // Check if any creation occupation matches expected occupations
  const hasMatchingCreationOccupation = normalizedCreationOccupations.some(
    (creationOcc) =>
      normalizedExpectedOccupations.some(
        (expectedOcc) =>
          creationOcc.includes(expectedOcc) ||
          expectedOcc.includes(creationOcc),
      ),
  );

  // Check if business occupation matches expected occupations
  const hasMatchingBusinessOccupation =
    normalizedBusinessOccupation &&
    normalizedExpectedOccupations.some(
      (expectedOcc) =>
        normalizedBusinessOccupation.includes(expectedOcc) ||
        expectedOcc.includes(normalizedBusinessOccupation),
    );
  ...
  return {
    isValid: false,
    message: `The work category "${category}" does not match your profile profession. Your profile shows: ${
      [...normalizedCreationOccupations, normalizedBusinessOccupation]
        .filter(Boolean)
        .join(", ") || "No profession set"
    }. Please ensure your profile profession matches the work you're uploading.`,
    expectedOccupations: normalizedExpectedOccupations,
    userOccupations: [
      ...normalizedCreationOccupations,
      normalizedBusinessOccupation,
    ].filter(Boolean),
  };
};
 ...
 * @param {boolean} isClient - Whether the user is a client (bypasses validation)
 * @returns {Object} Validation result
 */
const validateProfessionFields = (
  creationOccupation,
  businessOccupation,
  isClient = false,
) => {
  ...
  if (
    !isClient &&
    creationOccupationArray.length === 0 &&
    !businessOccupationTrimmed
  ) {
    return {
      isValid: false,
      message:
        "Please set your profession or occupation in your profile before uploading work. You must have at least one creation occupation or business occupation.",
    };
  }
...
const syncProfessionToUserModel = (userId, userSpaceData) => {
  ...
  if (
    Array.isArray(userSpaceData.creationOccupation) &&
    userSpaceData.creationOccupation.length > 0
  ) {
    professions.push(...userSpaceData.creationOccupation);
  }

  if (
    userSpaceData.businessOccupation &&
    userSpaceData.businessOccupation.trim()
  ) {
    professions.push(userSpaceData.businessOccupation);
  }

  // If user is a client and has no other professions, show as "Client"
  if (userSpaceData.isClient && professions.length === 0) {
    professions.push("Client");
  }
