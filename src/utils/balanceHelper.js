 ...
 */
const addPurchaseBalance = async (
  musicId,
  purchaseId,
  amount,
  ...
  try {
    ...
    const musicAsset = await ShareMusicAsset.findById(musicId).populate(
      "createdBy",
    );
    if (!musicAsset) {
    ...
    const platformFeePercent = 0.1; // 10% platform fee for music sales
 ...
 */
const deductPurchaseBalance = async (
  userId,
  amount,
  purchaseId,
  musicTitle = "",
